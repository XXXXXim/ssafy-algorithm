...
for tc in range(1,T+1):
    ...
    def backtracking(idx,selected,cost):
        ...
        # 1. 종료조건
        if idx ==N:
            # 지금까지 계산한 생산비용 cost 와
            # 이전 최소 생산비용 min_cost 중 작은 것 갱신
            min_cost= min(cost,min_cost)
            return
        
        # 2. 재귀호출
        #idx 제품을 몇번 공장에서 승할 것인지
        # 공장번호 j라 하면

        for j in range(N):
            #j번 공장은 이전에 다른 제품을 생산한적 없어야함
            # selcted통해 체크
            # j번 공장이 이전에 다른제품을 생산한적 없다면 idx번 제품을 생산하기로
            # idx+1번 제품을누가 생산할건지 정하러 재귀호출
